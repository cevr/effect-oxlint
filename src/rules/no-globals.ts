...
const memberBans: ReadonlyArray<MemberBan> = [
  ...
  [
    ...
    "Avoid console.* inside Effect context. Use Effect.log / Effect.logWarning / Effect.logError.",
  ],
  // Date
  ...
  [
    "JSON",
    ["parse", "stringify"],
    "Avoid JSON.parse/stringify inside Effect context. Use Schema.fromJsonString.",
  ],
  // Math.random
  [
    "Math",
    "random",
    "Avoid Math.random() inside Effect context. Use Random service from 'effect'.",
  ],
  // crypto
  [
    "crypto",
    ["randomUUID", "getRandomValues"],
    "Avoid crypto.randomUUID() inside Effect context. Use Random service from 'effect'.",
  ],
];
        ...
        Effect.flatMap(Ref.get(depth), (d) => {
          ...
          if (callee.type === "Identifier" && "name" in callee && callee.name === "Date") {
            return ctx.report(
              Diagnostic.make({
                node,
                message: "Avoid new Date() inside Effect context. Use Clock service from 'effect'.",
              }),
            );
          }
          return Effect.void;
